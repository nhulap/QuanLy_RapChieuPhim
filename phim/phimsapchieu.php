...
// Lấy danh sách phim Sắp Chiếu (> ngày hôm nay)
if($selectedTheLoai){
    $sql_sap_chieu = "SELECT * FROM phim WHERE NgayKhoiChieu > '$today' AND TheLoai LIKE '%$selectedTheLoai%' ORDER BY NgayKhoiChieu ASC";
} else {
    $sql_sap_chieu = "SELECT * FROM phim WHERE NgayKhoiChieu > '$today' ORDER BY NgayKhoiChieu ASC";
}
$result_sap_chieu = mysqli_query($conn, $sql_sap_chieu);
?>
...
<head>
    ...
    <style>
        ...
        .movie-info .btn-buy {
             ...
             background-color:#808080; /* Màu xám cho "Chưa Mở Bán" */
             color:white;
             padding:8px 15px;
             border-radius:3px;
             text-decoration:none;
             font-weight:bold;
             margin-top:5px; 
             cursor:default; /* Thay đổi con trỏ */
...
<div class="wrapper">
    ...
    <div class="main">
        ...
        <?php
        if(mysqli_num_rows($result_sap_chieu) > 0){
            while($row = mysqli_fetch_assoc($result_sap_chieu)){
                ...
                echo '<a class="btn-buy">Chưa Mở Bán</a>'; 
                echo '</div>';
                echo '</div>';
            }
        } else {
            echo "<p>Hiện tại không có phim sắp chiếu.</p>";
        }
        ?>
        </div>
    </div>
